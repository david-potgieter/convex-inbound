...
const inboundApiRateLimiter = new RateLimiter(components.rateLimiter, {
    inboundApi: {
        kind: "fixed window",
 ...
 */
async function scheduleBatchRun(ctx, apiKey) {
...
export const performBatchSend = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        if (!apiKey) {
            throw new Error("INBOUND_API_KEY is not set. Please set it using: " +
        ...
        for (const emailId of args.emailIds) {
            ...
            if (!limit.ok) {
                await new Promise((resolve) => setTimeout(resolve, limit.retryAfter ?? 100));
            }
            try {
